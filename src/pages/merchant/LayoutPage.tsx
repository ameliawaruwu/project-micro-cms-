      ...
      {pageMode === 'preview' && previewTemplate && (
        <div className="fixed inset-0 z-[60] bg-white flex flex-col font-sans animate-in fade-in duration-200">
          ...
          <div className="flex-1 overflow-y-auto bg-gray-100 flex justify-center py-4 sm:py-8 px-4 relative">
            ...
            <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[70] flex items-center gap-3 bg-white/95 backdrop-blur-md px-6 py-3 rounded-2xl shadow-2xl border border-gray-200/90 animate-in fade-in slide-in-from-bottom-4">
              <button
                onClick={() => { setPageMode('library'); setPreviewTemplate(null); }}
                className="px-5 py-2.5 rounded-xl border border-gray-300 text-xs font-semibold text-gray-700 hover:bg-gray-100 transition cursor-pointer"
              >
                Batal
              </button>
              <button
                onClick={() => handleApplyAndEdit(previewTemplate)}
                className="px-6 py-2.5 rounded-xl text-xs font-extrabold text-white bg-black hover:bg-gray-800 transition shadow-md hover:shadow-xl cursor-pointer flex items-center gap-2"
              >
