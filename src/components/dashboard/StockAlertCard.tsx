...
export const StockAlertCard: React.FC<StockAlertCardProps> = ({
  ...
  return (
    <div className="bg-white rounded-2xl border border-[#E5E0DD] p-4 sm:p-5 shadow-2xs font-sans text-left transition-all hover:border-[#66000E]/20">
      {/* Header */}
      <div className="flex items-center justify-between gap-3 pb-3 border-b border-[#E5E0DD]">
        ...
        <span
          className={`px-2.5 py-0.5 rounded-full text-xs font-medium border ${
            totalLowStock > 0
              ? 'bg-[#F5E8EA] text-[#66000E] border-[#E8DDDE]'
          ...
          lowStockItems.slice(0, 5).map((prod) => (
            <div key={prod.id} className="py-2.5 flex items-center justify-between gap-3 group">
              <div className="flex items-center gap-2.5 overflow-hidden min-w-0">
                <div className="relative shrink-0">
                  <img
                    ...
                    alt={prod.name}
                    className="w-10 h-10 rounded-xl object-cover border border-[#E8DDDE] shrink-0 bg-neutral-100"
                    referrerPolicy="no-referrer"
                  />
                  <span className="absolute -top-1 -right-1 w-2.5 h-2.5 rounded-full bg-[#66000E] border-2 border-white"></span>
                </div>
                <div className="truncate">
                  <p className="text-xs font-medium text-slate-800 truncate group-hover:text-[#66000E] transition-colors">
                    {prod.name}
                  </p>
              ...
              <div className="flex items-center gap-2 shrink-0">
                <span className="inline-block px-2.5 py-1 rounded-lg bg-[#F5E8EA] border border-[#E8DDDE] text-[#66000E] text-xs font-semibold">
                  {t('stock_remaining', 'Sisa')} {prod.stock}
                </span>
                <button
                  onClick={onManageStock}
                  title={isEn ? 'Restock this product' : 'Restock produk ini'}
                  className="p-1 rounded-lg hover:bg-[#F5E8EA] text-[#706866] hover:text-[#66000E] transition cursor-pointer"
                >
      ...
      <div className="pt-2">
        <button
          onClick={onManageStock}
          className="w-full py-2.5 min-h-[40px] rounded-xl bg-white hover:bg-[#F5E8EA]/70 border border-[#E5E0DD] hover:border-[#66000E]/40 text-[#66000E] font-medium text-xs flex items-center justify-center gap-2 transition cursor-pointer active:scale-98 shadow-2xs"
        >
