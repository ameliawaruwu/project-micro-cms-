...
export const ShareStoreModal: React.FC<ShareStoreModalProps> = ({
  ...
  const handleCopy = () => {
    ...
    setCopied(true);
    onShowNotification(t('link_copied_success', 'Link toko berhasil disalin ke clipboard!'));
  ...
  return (
    <div id="modal-share-store" className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-xs font-poppins">
      <div className="bg-white rounded-2xl max-w-md w-full p-6 sm:p-7 shadow-xl border border-[#E5E0DD] animate-in fade-in zoom-in duration-200">
        <div className="flex items-center justify-between pb-4 border-b border-[#E5E0DD]">
          <div className="flex items-center gap-2 text-[#1F1F1F] font-bold text-base">
            <Share2 className="w-5 h-5 text-[#66000E]" />
            <span>{t('share_store_title', 'Bagikan Toko Online')}</span>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="py-5 flex flex-col items-center text-center">
          ...
          <div className="p-4 bg-white rounded-2xl border border-[#E5E0DD] shadow-xs flex flex-col items-center mb-4">
            <div className="w-40 h-40 bg-[#F7F7F7] rounded-xl p-2 flex items-center justify-center relative overflow-hidden">
              <img
                src={`https://api.qrserver.com/v1/create-qr-code/?size=160x160&data=${encodeURIComponent(storeUrl)}&color=66000e`}
                alt="QR Code Toko"
                className="w-full h-full object-contain"
                referrerPolicy="no-referrer"
              />
            </div>
            <div className="flex items-center gap-1.5 mt-2.5 text-xs font-semibold text-[#66000E]">
              <QrCode className="w-3.5 h-3.5" />
              <span>{t('scan_to_open_mobile', 'Scan untuk Buka di HP')}</span>
            </div>
          </div>
          ...
          {/* Link Box */}
          <div className="w-full mt-4 p-2 bg-[#F7F7F7] border border-[#E5E0DD] rounded-xl flex items-center justify-between gap-2">
            <span className="text-xs font-mono text-[#1F1F1F] font-semibold truncate pl-2">{storeUrl}</span>
            <button
              onClick={handleCopy}
              className="flex items-center gap-1.5 px-3.5 py-2 rounded-lg bg-[#66000E] hover:bg-[#52000B] text-white text-xs font-semibold transition shrink-0 cursor-pointer shadow-xs"
            >
              {copied ? <Check className="w-3.5 h-3.5" /> : <Copy className="w-3.5 h-3.5" />}
              <span>{copied ? t('copied', 'Tersalin') : t('copy', 'Salin')}</span>
            </button>
          </div>
        </div>

        <div className="grid grid-cols-2 gap-3 pt-2 border-t border-[#E5E0DD]">
          <button
          ...
          >
            ...
            <span>{t('send_whatsapp', 'Kirim WhatsApp')}</span>
          </button>
          <a
            href={storeUrl}
            target="_blank"
          ...
          >
            ...
            <span>{t('visit_store', 'Kunjungi Toko')}</span>
          </a>
        </div>
      </div>
    </div>
